...
import { getMeetings, createMeeting } from "../redux/features/meetingThunks.js";
...
export default function Meetings() {
  ...
  const { meetings = [], isLoading, error } = useSelector((state) => state.meeting);
      ...
      <div className="w-full min-h-screen bg-[#FFFFFF] dark:bg-black flex flex-col items-center">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 w-full">
            ...
            <div className="flex lg:hidden items-center justify-between w-full">
              <div className="flex items-center gap-3">
                <button onClick={() => setSidebarOpen(true)} className="flex-shrink-0 p-1" aria-label="Open menu">
                  <FaBars className="text-2xl text-[#333] dark:text-gray-400" />
                </button>
                <h1 className="text-3xl sm:text-3xl font-bold text-[#000000] dark:text-[#F5F5F5]">Meetings</h1>
              </div>
              ...
              <button className="flex items-center gap-2 px-3 py-2 rounded-2xl shadow-[0_3px_10px_0_rgba(0,0,0,0.25)] dark:bg-[#383838] flex-shrink-0">
                ...
                <p className="text-xs sm:text-sm font-medium text-[#000000] dark:text-[#D5D5D5] whitespace-nowrap">Sync Calendar</p>
              </button>
            </div>

            
            <div className="hidden lg:flex flex-col gap-4 flex-1">
              <h1 className="text-2xl font-bold text-[#000000] dark:text-[#F5F5F5]">Meetings</h1>
              <p className="text-lg text-[#000000] dark:text-[#F5F5F5]">Manage your schedule and prepare for upcoming calls</p>
            </div>
            ...
            <div className="lg:hidden flex flex-col gap-3">
              <div className="flex items-center justify-between w-full gap-2">
                <div className="flex-1 min-w-0">
                  <FilterTabs activeFilter={activeFilter} setActiveFilter={handleFilterChange} />
                </div>
              ...
              <div className="w-full">
                <div className="flex items-center px-4 py-2.5 rounded-[30px] bg-[#EDEDED] dark:bg-[#383838] border border-[#E0DDDD] w-full">
                  ...
                  <input type="text" placeholder="Search meetings, documents..." className="w-full text-sm text-[#393838] dark:text-[#D2D2D2] placeholder:text-sm bg-transparent dark:placeholder:text-[#D5D5D5] outline-none border-none pl-3" />
                </div>
              </div>
            </div>
        ...
        {isLoading && (
          <div className="flex justify-center items-center py-20">
            <p className="text-[#393838] dark:text-[#D2D2D2] text-sm">Loading meetings...</p>
          </div>
        )}
        {error && !isLoading && (
          <div className="flex justify-center items-center py-20">
            <p className="text-red-500 text-sm">Failed to load meetings. Please try again.</p>
          </div>
        )}
        {!isLoading && !error && filteredMeetings.length === 0 && (
          <div className="flex justify-center items-center py-20">
            <p className="text-[#393838] dark:text-[#D2D2D2] text-sm">No meetings found.</p>
          </div>
        )}
